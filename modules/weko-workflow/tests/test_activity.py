...
from weko_workflow.api import ActivityStatusPolicy
...
class TestHeadlessActivity:
    ...
    def test_init_activity(self, app, db, workflow, users, client):
        with patch(
            "weko_workflow.views.WorkActivity.get_new_activity_id"
        ) as mock_get_new_activity_id:
            mock_get_new_activity_id.side_effect = [
                f"A-TEST-0000{i}" for i in range(1, 20)
            ]
            ...
            activity.init_activity(
                users[0]["id"], workflow["workflow"].id, community="comm01"
            )
            ...
            assert (
                activity.detail
                == "http://test_server.localdomain/workflow/activity/detail/A-TEST-00002?community=comm01"
            )
        ...
        with patch(
            "weko_workflow.headless.activity.init_activity"
        ) as mock_init_activity:
        ...
        with patch(
            "weko_workflow.headless.activity.verify_deletion"
        ) as mock_verify_deletion:
            mock_verify_deletion.return_value = jsonify({"is_delete": True})
            with pytest.raises(WekoWorkflowException) as ex:
                activity.init_activity(
                    users[1]["id"], workflow["workflow"].id, activity_id="A-TEST-00001"
                )
        ...
        with patch(
            ...
            ) as mock_get_activity_by_id:
                mock_get_activity_by_id.return_value = None
                with pytest.raises(WekoWorkflowException) as ex:
                    activity.init_activity(
                        users[1]["id"],
                        workflow["workflow"].id,
                        activity_id="A-TEST-00001",
                    )
            ...
            with patch(
                ...
                mock_get_activity_by_id.return_value = MagicMock(
                    activity_login_user=users[1]["id"], shared_user_id=users[1]["id"]
                )
                with patch(
                    "weko_workflow.utils.check_authority_by_admin", return_value=False
                ):
                    with pytest.raises(WekoWorkflowException) as ex:
                        activity.init_activity(
                            users[0]["id"],
                    ...
                    assert (
                        str(ex.value)
                        == f"user({users[0]['id']}) cannot restart activity(A-TEST-00001)."
                    )
    ...
    def test_item_registration(self, app, db, workflow, users, client, mocker):
        ...
        with patch(
            "weko_workflow.headless.activity.check_validation_error_msg"
        ) as mock_validation_msg:
            msg = ["error<br/>title"]
            error_list = {"mapping": {"title": "error"}}
            mock_validation_msg.return_value = jsonify(
                code=1, msg=msg, error_list=error_list
            )
            with pytest.raises(WekoWorkflowException) as ex:
                activity.item_registration(metadata={}, files=[], index=[], comment="")
        ...
        mock_get_new_activity_id = mocker.patch(
            "weko_workflow.views.WorkActivity.get_new_activity_id"
        )
        ...
        with patch(
            "weko_workflow.headless.activity.HeadlessActivity._input_metadata",
            return_value=200001,
        ):
    ...
    def test__input_metadata(self, app, db, workflow, users, client, mocker):
        mocker.patch.object(
            HeadlessActivity,
            "_upload_files",
            return_value=[{"file_name": "test.txt", "file_id": "12345"}],
        )
        mocker.patch(
            "weko_workflow.api.WorkActivity.upt_activity_metadata", return_value=None
        )
        mocker.patch(
            "weko_workflow.headless.activity.get_workflow_journal", return_value=None
        )
        mocker.patch(
            "weko_workflow.headless.activity.get_feedback_maillist",
            return_value=(MagicMock(json={"code": 1, "data": []}), None),
        )
        mocker.patch(
            "weko_workflow.headless.activity.get_mapping",
            return_value={"title.@value": "title"},
        )
        mocker.patch(
            "weko_workflow.headless.activity.get_data_by_property",
            return_value=(["Test Title"], None),
        )
        mocker.patch(
            "weko_workflow.headless.activity.current_pidstore.minters",
            {
                "weko_deposit_minter": lambda record_uuid, data: MagicMock(
                    pid_value="200001"
                )
            },
        )
        mocker.patch(
            "weko_workflow.headless.activity.WekoDeposit.create",
        ...
        with patch(
            ...
            # case 1
            mocker.patch(
                "weko_workflow.headless.activity.validate_form_input_data",
                side_effect=lambda result, itemtype_id, metadata: result.update(
                    {"is_valid": True}
                ),
            )
            mock_get_new_activity_id.side_effect = [
            ...
            activity.init_activity(
                users[1]["id"], workflow["workflow"].id, community="comm01"
            )
            ...
            mocker.patch(
                "weko_workflow.headless.activity.validate_form_input_data",
                side_effect=lambda result, itemtype_id, metadata: result.update(
                    {"is_valid": False, "error": "Invalid metadata"}
                ),
            )
            with pytest.raises(WekoWorkflowException) as ex:
            ...
            mocker.patch(
                "weko_workflow.api.WorkActivity.upt_activity_metadata",
                side_effect=Exception("upt_activity_metadata error"),
            )
            with pytest.raises(WekoWorkflowException) as ex:
                activity._input_metadata(metadata, files)
            assert (
                str(ex.value) == "failed to input metadata: upt_activity_metadata error"
            )
    ...
    def test__upload_files(self, app, db, workflow, users, client, mocker):
        ...
        mocker.patch(
            "weko_workflow.headless.activity.ObjectVersion.create",
            return_value=mock_object_version,
        )
        ...
        with pytest.raises(FileSizeError) as ex:
            activity._upload_files(
                [
                    MagicMock(
                        filename="test.txt",
                        stream=mock_open(read_data="data"),
                        content_length=501,
                    )
                ]
            )
    ...
    def test__user_lock(self, app, db, workflow, users, client, mocker):
        ...
        mock_current_cache_get = mocker.patch(
            "weko_workflow.headless.activity.current_cache.get", return_value=None
        )
        mock_update_cache_data = mocker.patch(
            "weko_workflow.headless.activity.update_cache_data"
        )
        ...
        mock_current_cache_get.assert_called_once_with(
            "workflow_userlock_activity_{}".format(str(activity.user.id))
        )
        ...
        mock_update_cache_data.assert_called_once_with(
            "workflow_userlock_activity_{}".format(str(activity.user.id)),
            activity.activity_id,
            3600,
        )
    ...
    def test__user_unlock(self, app, db, workflow, users, client, mocker):
        ...
        # Mocking the necessary methods and properties
        mock_delete_user_lock_activity_cache = mocker.patch(
            "weko_workflow.headless.activity.delete_user_lock_activity_cache"
        )
        ...
        mock_delete_user_lock_activity_cache.assert_called_once_with(
    ...
    def test__activity_lock(self, app, db, workflow, users, client, mocker):
        ...
        activity._lock_skip = False
        mock_lock_activity = mocker.patch(
            "weko_workflow.headless.activity.lock_activity",
            return_value=(
                MagicMock(get_json=lambda: {"locked_value": "test_locked_value"}),
                None,
            ),
        )
        result = activity._activity_lock()
    ...
    def test__activity_unlock(self, app, db, workflow, users, client, mocker):
        ...
        # Mocking the necessary methods and properties
        mock_delete_lock_activity_cache = mocker.patch(
            "weko_workflow.headless.activity.delete_lock_activity_cache"
        )
        ...
        activity._lock_skip = False
        result = activity._activity_unlock("test_locked_value")
        mock_delete_lock_activity_cache.assert_called_once_with(
            activity.activity_id, {"locked_value": "test_locked_value"}
        )
